...
//※※※※※※ AxxBxx命名对应数据库axx_bxx
	Class ProRevModel extends Model{
		Public function PtoR($majortab,$majid,$minortab,$minid,$middletab,$resultKfield,$resultVfield){
 				...
 				foreach ($id2 as $v) {
 					...
 					foreach ($col as $v) {
						$result["$k"]["$i"] = $v["$resultVfield"];		
 						$where3['reviews'] = $v["$resultVfield"];
 						$aid = M('reviews')->where($where3)->select();
 						foreach ($aid as $v) {
 							$result["$k"]["$i"] .= ','.$v['time'];
 							$where4['id'] = $v['authorid'];
 							$ema = M('homeuser')->where($where4)->select();
 							foreach ($ema as $v) {
 								$result["$k"]["$i"] .= ','.$v['email'];
 							}
 						}						
