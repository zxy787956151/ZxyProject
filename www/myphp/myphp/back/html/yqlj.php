	...
	<div class="shell">
		...
		<div id="navigation">
			<ul>
			    ...
			    <li><a href="#" class="active"><span>友情链接</span></a></li>
			</ul>
		</div>
		<!-- End Main Nav -->
	</div>
					...
					<div class="table">
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							...
							<?php
								...
								if (@$_GET['page']=="") {
									$query="select * from yqlj limit 0,5;"; //定义sql
								}
								if (@$_GET['page']=='2') {
									$query="select * from yqlj limit 5,5;"; //定义sql
								}
								if (@$_GET['page']=='3') {
									$query="select * from yqlj limit 10,5;"; //定义sql
								}
								...
								while($rows=mysql_fetch_array($result)){
									...
									echo "<tr class='odd'>
											...
											<td><a href='deleten.phpclass='ico del'>Delete</a><a href='#' class='ico edit'>Edit</a></td>
										</tr>";
								}
				...
				<div class="box">
					...
					<form action="" method="post">
						...
						<div class="form">
								...
								<p>
									<span class="req">max 100 symbols</span>
									<label>href<span>(Required Field)</span></label>Content
	...
	if (@$_POST['sub']=='submit') {
		if (@$_POST['title']=="") {
			...
			$pdo = new PDO("mysql:host=localhost;dbname=bk","root","");
				$pdo  -> exec("insert into yqlj(title)values('$title');");
				
		
			header('Location:header7.php');
		}
