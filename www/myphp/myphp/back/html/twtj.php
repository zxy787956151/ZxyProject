	...
	<div class="shell">
		...
		<div id="navigation">
			<ul>
			    ...
			    <li><a href="twtj.php" class="active"><span>图文推荐</span></a></li>
			</ul>
		</div>
		<!-- End Main Nav -->
	</div>
					...
					<div class="table">
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							...
							<?php
								...
								if (@$_GET['page']=="") {
									$query="select * from twtj limit 0,5;"; //定义sql
								}
								if (@$_GET['page']=='2') {
									$query="select * from twtj limit 5,5;"; //定义sql
								}
								if (@$_GET['page']=='3') {
									$query="select * from twtj limit 10,5;"; //定义sql
								}
								...
								while($rows=mysql_fetch_array($result)){
									...
									echo "<tr class='odd'>
											...
											<td><a href='delete3.php' class='ico del'>Delete</a><a href='#' class='ico edit'>Edit</a></td>
										</tr>";
								}
				...
				<div class="box">
					...
					<form action="" method="post">
						...
						<div class="form">
								...
								<p>
									<span class="req">max 100 symbols</span>
									<label>Kind<span>(Required Field)</span></label>Kind
									<textarea name="kind" class="field size5" rows="2" cols="15"></textarea>
								</p>	
	...
	if (@$_POST['sub']=='submit') {
		if (@$_POST['title']=="") {
			...
			$pdo = new PDO("mysql:host=localhost;dbname=bk","root","");
				$pdo  -> exec("insert into twtj(img,title,kind,time)values('images/$img','$title','$kind',now());");
