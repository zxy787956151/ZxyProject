...
<div id="header">
	<div class="shell">
		...
		<div id="navigation">
			<ul>
			    <li><a href="#" class="active"><span>文章</span></a></li>
			    ...
			    <li><a href="bqy.php"><span>标签云</span></a></li>
			    <li><a href="twtj.php"><span>图文推荐</span></a></li>
			    <li><a href="yqlj.php"><span>友情链接</span></a></li>
			</ul>
		</div>
		<!-- End Main Nav -->
	</div>
					...
					<div class="table">
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							...
							<?php
								...
								if (@$_GET['page']=="") {
									$query="select * from mytext limit 0,5;"; //定义sql
								}
								if (@$_GET['page']=='2') {
									$query="select * from mytext limit 5,5;"; //定义sql
								}
								if (@$_GET['page']=='3') {
									$query="select * from mytext limit 10,5;"; //定义sql
								}
								...
								while($rows=mysql_fetch_array($result)){
									...
									echo "<tr class='odd'>
											...
											<td><a href='delete.php?id=$id&img_id=$img_id' class='ico del'>Delete</a><a href='#' class='ico edit'>Edit</a></td>
										</tr>";
								}
	...
	if (@$_POST['sub']=='submit') {
		if (@$_POST['title']=="") {
			...
			$pdo = new PDO("mysql:host=localhost;dbname=bk","root","");
				$pdo  -> exec("insert into mytext(title,time,content)values('$title',now(),'$content');");
				$pdo  -> exec("insert into img(href)values('images/$img')");
				$sql="select * from img where href='images/$img'";
				$res=$pdo->query($sql);
				foreach ($res as $rows) {
					$img_id=$rows['id'];
				}
				$pdo  -> exec("update mytext set img_id='$img_id' where title='$title'");
				var_dump($img_id);
