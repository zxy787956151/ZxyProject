...
$(function(){
	$(".btn").click(function(){		//live 发生某个事件（ click事件） 
		...
		$.ajax({	
			...
			url: "http://my.php.com/StudySystem/index.php/Admin/Index/ajax",
			...
			success: function(json){
				if(json.success==1){
					...
					if (json.sub===1) {
						...
						$("#warnland").css("display","none");
						$("#userpic").css("display","block");
						$("#userwel").css("display","block");
						$('.landing').fadeOut(100);
						$('#userwel p').slice(1).html('aaa');
						
					}else{
						$("#result").remove();
						var span = "<br/><span style='color:red' id='result'>"+json.result+"</span>";
			    		$("#btn").append(span);
					}
