<?php 
	Class MySQLiSessionHandler {
		...
		public function __construct($host, $user, $pswd, $db, $_sessionName, $_sessionTable){
			$this->_dbLink = new mysqli($host, $user, $pswd, $db);
			$this->_sessionName = $_sessionName;
			$this->_sessionTable = $_sessionTable;

			session_set_sava_handler(
		...
		function session_write($SID, $value){
			$stmt = $this->_dbLink->prepare("
				...
				(sid, value) VALUES (?, ?) ON DUPLTCATE KEY
		...
		function session_read($SID){
			$stmt = $this->_dbLink->prepare("
				...
				WHERE sid = ? AND
				UNIX_TIMESTAMP(expiration) + " . self::SESS_EXPIRE ."> UNIX_TIMESTAMP(NOW())			
				");
		...
		public function session_gc($lifetime){
			$stmt = $this->_dbLink->prepare("DELETE FROM {$this->_sessionTable} WHERE UNIX_TIMESTAMP(expiration) < " . UNIX_TIMESTAMP(NOW()) - self::SESS_EXPIRE);
			$stmt->execute();
		}
