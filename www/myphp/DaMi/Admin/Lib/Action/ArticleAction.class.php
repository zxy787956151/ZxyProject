...
{	
    ...
    {	
		...
		empty($_POST['description']) ? $data['description'] = msubstr(strip_tags($_POST['content']),0,100) : $data['description'] = trim($_POST['description']);
	...
	public function delall()
	{
		...
		if($_REQUEST['Del'] == '移动')
		{
		if(intval($_REQUEST['typeid']) ==0){$this->error('操作失败,请选择目标类别！');}
	...
	private function addop(){
		...
		$where .= '1=1';
		$list = $type->where($where)->field("typeid,typename,fid,concat(path,'-',typeid) as bpath")->order('bpath')->select();
		foreach($list as $k=>$v)
		{
