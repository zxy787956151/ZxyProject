    ...
    static public function AccessDecision($appName=APP_NAME)
    {
        ...
        if(RBAC::checkAccess()) {
            ...
            if(empty($_SESSION[C('ADMIN_AUTH_KEY')])) {
				...
				$sql    =   "select role.typeids from ".
                    $table['role']." as role,".
                    $table['user']." as user".
                    " where user.user_id=".$_SESSION[C('USER_AUTH_KEY')]." and user.role_id=role.id and role.status=1";
					//echo $sql;
                $rs =   M()->query($sql);
				if($rs){$_SESSION[C('USER_CONTENT_KEY')]=$rs[0]['typeids'];
				}else{$_SESSION[C('USER_CONTENT_KEY')]='';}
				}
				...
				if(!$t2){$_SESSION[$accessGuid]=false;return false;}
