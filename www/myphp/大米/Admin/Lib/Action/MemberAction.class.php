...
//支付宝配置
    function doap(){
	if(empty($_POST['ap_email']) || empty($_POST['ap_pid']) || empty($_POST['ap_key'])){$this->error('配置参数不能为空');}
	//改写配置文件
...
//微信基础配置
    function do_wxset(){
	if(empty($_POST['wx_appid']) || empty($_POST['wx_appkey']) || empty($_POST['wx_token']) || empty($_POST['wx_jqrkey'])){$this->error('配置参数不能为空');}
	//改写配置文件
	$config_file = "./Public/Config/config.ini.php";
	$fp = fopen($config_file,"r");
	$configStr = fread($fp,filesize($config_file));
...
if($_POST){
 ...
 //一级菜单
   ...
   if(!empty($_POST['value_0_'.$i])){
	  ...
	  if(!$t){$dao->add($data);}else{$dao->where('id='.$i)->save($data);}
   }else{
      //增加顶级 
	  $data['id'] = $i;   
	  $data['menu_name'] = $_POST['key_'.$i];
	  ...
	  if(!$t){$dao->add($data);}else{$dao->where('id='.$index)->save($data);}
	   }
	   }
   }
 }
...
if($id !=0){M('wx_menu')->where('id='.$id)->delete();}
}
...
if($buyid==0){$this->ajaxReturn('参数错误','提示',0);exit();}
...
function send_mail(){
if($_POST){
if($_POST['title']=='' || $_POST['content'] == ''){$this->error('请输入标题和内容!');exit();}
$where = '';
...
if(!$info){$this->error('信息不存在!');exit();}
M('tixian')->where('id='.$id)->setField('status',1);
M('member')->setDec('money','id='.$info['uid'],$info['money']);
$this->success('提现处理成功!!');
}
//导出会员到excel
function goexcel(){
...
for($i=0;$i<$dataNum;$i++){
		  ...
		  if($k == 'addtime' &&  $data_value != ''){$data_value = date('Y-m-d H:i:s',$data_value);} //对注册时间特殊处理下
